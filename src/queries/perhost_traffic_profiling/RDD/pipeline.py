...
def run(spark: SparkSession, parquet_path: str) -> dict:
    ...
    return {
        "requests_per_host": requests_per_host.collect(),
        "bytes_per_host": bytes_per_host.collect(),
        "average_bytes_per_host": average_bytes_per_host.collect(),
        "error_rate_per_host": error_rate_per_host.collect(),
        "distinct_endpoints_per_host": distinct_endpoints_per_host.collect()
    }
...
if __name__ == "__main__":
    ...
    for host, count in results["requests_per_host"][:10]:
        print(f"{get_hostname(host)}: {count}")

    print("\nTotal bytes sent per host:")
    for host, bytes_sent in results["bytes_per_host"][:10]:
        print(f"{get_hostname(host)}: {bytes_sent}")

    print("\nAverage bytes per request per host:")
    for host, avg_bytes in results["average_bytes_per_host"][:10]:
        print(f"{get_hostname(host)}: {avg_bytes:.2f}")

    print("\nError rate per host:")
    for host, error_rate in results["error_rate_per_host"][:10]:
        print(f"{get_hostname(host)}: {error_rate:.2f}%")

    print("\nDistinct endpoints accessed per host:")
    for host, distinct_endpoints in results["distinct_endpoints_per_host"][:10]:

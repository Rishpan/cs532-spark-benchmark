# Visualizes the average executor CPU time for the "perhost_traffic_profiling" query across different data scales and APIs
...
# Visualized as a line plot, where the x-axis represents the data scale percentage and the y-axis represents the CPU time in seconds
# Different lines for each API
import json
import pandas as pd
...
ax.set_ylabel("Executor CPU Time (sec)")
ax.set_title("CPU Time Scaling — perhost_traffic_profiling")
ax.legend(title="API")
plt.tight_layout()
plt.savefig("../figures/cpu_scaling_perhost.png", dpi=150, bbox_inches="tight")
plt.show()
